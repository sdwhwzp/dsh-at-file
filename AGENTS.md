...
- Harness 0.1.7 persists the picker preferences in the Loader row configuration under `preferences`; writes use the settings form for that exact row. Browser reads and writes MUST use `atFile/getSettings` and `atFile/updateSettings`; the Host methods own normalization and call the owner settings scope. This keeps the package self-contained and preserves the web profile's durable configuration.
