...
Harness 0.1.7 stores picker preferences under the plugin profile row’s `preferences` field. When upgrading, copy the old `at-file` settings section into that field; the dedicated `atFile` RPCs retain their existing API.
...
Use the same command to update an existing installation. Restart `dsh web` after installation so the Host and browser client load version `0.7.4-dsh.20260923.1`.

Version `0.7.4-dsh.20260923.1` targets Harness `0.1.7-alpha.2` and declares its Harness package peers as `^0.1.7-alpha.2`. `dsh-client-store` is the declared client state peer; the bundled compatibility fallback does not require `dsh-client-runtime` to be installed separately.
...
The development setup expects a matching DeepSeek Harness `0.1.7-alpha.2` checkout at `../deepseek-harness`, its default clone directory. The browser bundle uses the split `dsh-client-store` platform module, and the settings owner updates the Loader row containing its `preferences`. Built files under `lib/` are committed so profile installation does not require package build scripts.
